...
### =======================================================================
### 
### Plot the field profiles in the Oxz plane 
### 
### =======================================================================
...
for f in efields:
    ...
    # Take the slice (band,x,z) for Ey at y = 0
    Ey = f[:,yindex,:,1]
    ...
    # Take the slice (band,x,z) for Ez at y = 0
    Ez = f[:,yindex,:,2]
...
Xfield, Zfield = np.meshgrid( np.linspace(-0.5,0.5,resolution_field),
...
for i in range(8):
    ...
    axs[0].contour(X,Z,eps_Oxz.T,cmap='binary') 
    axs[0].pcolormesh(Xfield,Zfield,Ex.T,shading='gouraud',cmap='RdBu')
    ...
    axs[1].contour(X,Z,eps_Oxz.T,cmap='binary') 
    axs[1].pcolormesh(Xfield,Zfield,Ey.T,shading='gouraud',cmap='RdBu') 
    ...
    axs[2].contour(X,Z,eps_Oxz.T,cmap='binary') 
    axs[2].pcolormesh(Xfield,Zfield,Ez.T,shading='gouraud',cmap='RdBu')
    ...
    plt.savefig('E_Oxz_Band'+str(i+1)+'.png')
...
for f in hfields:
    ...
    # Take the slice (band,x,z) for Hx at y = 0
    Hx = f[:,yindex,:,0]
    ...
    # Take the slice (band,x,z) for Hy at y = 0
    Hy = f[:,yindex,:,1]
    ...
    # Take the slice (band,x,z) for Hz at y = 0
    Hz = f[:,yindex,:,2]
...
for i in range(8):
    ...
    fig, axs = plt.subplots(1,3)  
    axs[0].contour(X,Z,eps_Oxz.T,cmap='binary') 
    axs[0].pcolormesh(Xfield,Zfield,Hx.T,shading='gouraud',cmap='RdBu')
    axs[0].set_xticks(np.linspace(-0.5,0.5,2)) 
    axs[0].set_yticks(np.linspace(-Zlim,Zlim,int(Lz)+1)) 
    axs[0].set_xlabel('x/a',fontsize=14)
    axs[0].set_ylabel('z/a',fontsize=14) 
    ...
    axs[1].contour(X,Z,eps_Oxz.T,cmap='binary') 
    axs[1].pcolormesh(Xfield,Zfield,Hy.T,shading='gouraud',cmap='RdBu') 
    axs[1].set_xticks(np.linspace(-0.5,0.5,2)) 
    axs[1].set_yticks(np.linspace(-Zlim,Zlim,int(Lz)+1)) 
    axs[1].set_xlabel('x/a', fontsize=14)
    #axs[1].set_ylabel('z/a',fontsize=14) 
    ...
    axs[2].contour(X,Z,eps_Oxz.T,cmap='binary') 
    axs[2].pcolormesh(Xfield,Zfield,Hz.T,shading='gouraud',cmap='RdBu')
    axs[2].set_xticks(np.linspace(-0.5,0.5,2)) 
    axs[2].set_yticks(np.linspace(-Zlim,Zlim,int(Lz)+1)) 
    axs[2].set_xlabel('x/a', fontsize=14)
    #axs[2].set_ylabel('z/a', fontsize=14) 
    ...
    fig.suptitle('Band '+str(i+1)+': y = 0', fontsize=14)  
    plt.savefig('H_Oxz_Band'+str(i+1)+'.png')
