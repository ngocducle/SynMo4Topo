...
def Hamiltonian(k,q,omega1,v1,U1,W1,alpha1,beta1,
                omega2,v2,U2,W2,alpha2,beta2,V):
    ...
    ### Block (1,1)
    Hamiltonian[0,0] = omega1 + v1*k + 0.5*beta1*k**2
    Hamiltonian[0,1] = W1 
    Hamiltonian[0,2] = W1 
    Hamiltonian[0,3] = U1p - 0.5*beta1*k**2 

    Hamiltonian[1,0] = W1
    Hamiltonian[1,1] = omega1 - 0.5*beta1*k**2 
    Hamiltonian[1,2] = U1m + 0.5*beta1*k**2 
    Hamiltonian[1,3] = W1 

    Hamiltonian[2,0] = W1 
    Hamiltonian[2,1] = U1m + 0.5*beta1*k**2 
    Hamiltonian[2,2] = omega1 - 0.5*beta1*k**2 
    Hamiltonian[2,3] = W1 

    Hamiltonian[3,0] = U1p - 0.5*beta1*k**2 
    Hamiltonian[3,1] = W1 
    Hamiltonian[3,2] = W1 
    Hamiltonian[3,3] = omega1 - v1*k + 0.5*beta1*k**2 
    ...
    ### Block (2,2)
    Hamiltonian[4,4] = omega2 + v2*k + 0.5*beta2*k**2 
    Hamiltonian[4,5] = -W2*cmath.exp(-1j*K*q) 
    Hamiltonian[4,6] = -W2*cmath.exp(-1j*K*q) 
    Hamiltonian[4,7] = (U2p - 0.5*beta2*k**2 )*cmath.exp(-1j*2*K*q) 

    Hamiltonian[5,4] = -W2*cmath.exp(1j*K*q)
    Hamiltonian[5,5] = omega2 - 0.5*beta2*k**2 
    Hamiltonian[5,6] = U2m + 0.5*beta2*k**2  
    Hamiltonian[5,7] = -W2*cmath.exp(-1j*K*q) 

    Hamiltonian[6,4] = -W2*cmath.exp(1j*K*q) 
    Hamiltonian[6,5] = U2m + 0.5*beta2*k**2 
    Hamiltonian[6,6] = omega2 - 0.5*beta2*k**2
    Hamiltonian[6,7] = -W2*cmath.exp(-1j*K*q) 

    Hamiltonian[7,4] = (U2p - 0.5*beta2*k**2)*cmath.exp(1j*2*K*q) 
    Hamiltonian[7,5] = -W2*cmath.exp(1j*K*q) 
    Hamiltonian[7,6] = -W2*cmath.exp(1j*K*q) 
    Hamiltonian[7,7] = omega2 - v2*k + 0.5*beta2*k**2
...
def dH_q(k,q,W2,alpha2,beta2,V):
    ...
    dHq[4,7] = -1j*2*K*(U2*(1+alpha2)-0.5*beta2*k**2)*cmath.exp(-1j*2*K*q)
    ...
    dHq[7,4] = 1j*2*K*(U2*(1+alpha2)-0.5*beta2*k**2)*cmath.exp(1j*2*K*q)
...
for ik in range(Nk):
    ...
    dHk = dH_k(k,v1,alpha1,v2,alpha2)
    ...
    for iq in range(Nq):
        ...
        ### The Hamiltonian 
        H = Hamiltonian(k,q,omega1,v1,U1,W1,alpha1,beta1,
                        omega2,v2,U2,W2,alpha2,beta2,V) 

        ### The derivative dH/ddelta 
        dHq = dH_q(k,q,W2,alpha2,beta2,V)
