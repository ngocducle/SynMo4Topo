...
### ======================================================================
###
### Plot the E-field profiles in the Oxy plane
### 
### ======================================================================
...
Lz = 10      # Length of the unit cell along the z-direction

### The value of z where we take a slice to plot the field
zvalue = 0.5*(dist+h)+0.25*h
print('z = '+str(zvalue))
...
geometry = [
    ...
    mp.Block(center = mp.Vector3(0.0,0.0,0.5*(h+dist)),
    ...
    mp.Cylinder(center = mp.Vector3(0.5*delta1,0.5*delta2,0.5*(dist+h)),
                radius = radius,
                height = h,
...
md_eps = mpb.MPBData(rectify = True,
                 periods = num_periods,
                 resolution = resolution_eps)
eps = ms.get_epsilon()
converted_eps = md_eps.convert(eps)
...
# The indexx of the slice in the z-axis
...
X, Y = np.meshgrid(
    np.linspace(-Xlim,Xlim,Nx), np.linspace(-Ylim,Ylim,Ny)
)

### Get the fields
resolution_field = 81
md_field = mpb.MPBData(rectify = True,
...
for f in efields:
    # Get the x component of the E-field
    print('The shape of f: '+str(np.shape(f)))

    # The the slice (band,x,y) for Ex at z
    Ex = f[:,:,zindex_field,0]

    # Save the data Ex to Efieldsx
    Efieldx.append(md_field.convert(Ex))

    # Get the y component of the E-field
    print('The shape of f: '+str(np.shape(f)))

    # The the slice (band,x,y) for Ey at z
    Ey = f[:,:,zindex_field,1]

    # Save the data Ey to Efieldsy
    Efieldy.append(md_field.convert(Ey))

    # Get the z component of the E-field
    print('The shape of f: '+str(np.shape(f)))

    # The the slice (band,x,y) for Ez at z
    Ez = f[:,:,zindex_field,2]

    # Save the data Ez to Efieldsz
    Efieldz.append(md_field.convert(Ez))
